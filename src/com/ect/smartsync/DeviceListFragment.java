...
//		}
		...
		Object[] devices = peerList.getDeviceList().toArray();
		for( int i = 0 ; i<devices.length ; i++ ){
			WifiP2pDevice dev = (WifiP2pDevice)devices[i];
			Log.v("DSYOU", "======= nm: " + dev.deviceName + ", addr: " + dev.deviceAddress + ",state: " + dev.status);
			
			DeviceDetailFragment.peer_macaddr = dev.deviceAddress;//dsyou....
			DeviceDetailFragment.peer_name    = dev.deviceName;//dsyou....
			
		}
//		Log.v("SAPARK", "============================================= onPeersAvailable()" + peerList.getDeviceList().size());
//		Log.v("SAPARK", "============================================= onPeersAvailable()" + peerList.describeContents());
		if (peers.size() == 0) {
	...
	public void onInitiateDiscovery() {
		...
		progressDialog = new SmartSyncProgressDialog(getActivity());
...
//				});
	...
	public void closeProgress() {
		if (progressDialog != null && progressDialog.isShowing()) {
			Log.v("SAPARK", "============================================= onCancelDiscovery(true)");
			progressDialog.dismiss();
		}
	}
